    ...
    <article class={`@container bg-black text-neutral border border-primary rounded-md flex flex-wrap items-center ${member.name === selected && 'scale-110'}`}>
      {member.imageUrl && (
        <img
          ...
          height={160}
          width={280}
          class="rounded-md w-full h-24 @card:h-auto @card-showcase:w-2/5 @card-showcase:h-[200px] @card-showcase:md:h-[300px] object-cover"
        />
      )}
      <div class="hidden p-4 lg:p-6 @card:flex flex-col gap-3 lg:gap-6 w-full @card-showcase:w-3/5">
        ...
        <footer class="flex gap-x-2 lg:gap-x-4">
          {member.xUrl && (<a href={member.xUrl} aria-label={`Link to the Twitter of ${member.name}`} target="_blank">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
          ...
          {member.linkedInUrl && (<a href={member.linkedInUrl} aria-label={`Link to the LinkedIn of ${member.name}`} target="_blank">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
          ...
          {member.gitHubUrl && (<a href={member.gitHubUrl} aria-label={`Link to the GitHub of ${member.name}`} target="_blank">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
